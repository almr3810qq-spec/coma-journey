...
// Run scheduler every 15 seconds to deliver traffic smoothly
setInterval(processInteractionSchedules, 15000);
...
app.get("/api/books/download/:id", (req, res) => {
  ...
  try {
    // Increment download counter safely: count each unique visitor/device ONLY ONCE per book edition
    const clientIp = (req.headers["x-forwarded-for"] as string)?.split(",")[0]?.trim() || req.ip || req.socket.remoteAddress || "127.0.0.1";
    
    // Check or set visitor cookie
    let visitorId = req.headers["cookie"]?.split("; ")?.find(row => row.startsWith("coma_visitor_id="))?.split("=")[1];
    if (!visitorId) {
      visitorId = `v_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
      res.setHeader("Set-Cookie", `coma_visitor_id=${visitorId}; Path=/; Max-Age=31536000; SameSite=Lax`);
    }

    const visitorKey = `${visitorId}_${clientIp}`;

    if (!book.downloadedBy) {
      book.downloadedBy = [];
    }

    if (!book.downloadedBy.includes(visitorKey)) {
      book.downloadedBy.push(visitorKey);
...
// 11.2. Direct Media Upload Endpoint (Videos & Images)
app.post("/api/upload-media", (req, res) => {
  ...
  try {
    ...
    const safeName = `media_${Date.now()}_${Math.random().toString(36).substring(2, 8)}${cleanExt}`;

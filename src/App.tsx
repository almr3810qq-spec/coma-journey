...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (uiTexts.disablePostsNotifications === true) {
  ...
  // Deep URL & Hash route parser for functional shared post links (#post-xxx or ?post=xxx)
  useEffect(() => {
    const handleUrlRouting = () => {
      ...
      if (hash.startsWith('#post-')) {
        targetPostId = hash.replace('#post-', '');
      } else if (postParam) {
        targetPostId = postParam;
      }

      if (targetPostId) {
        if (!uiTexts.hidePostsSection) {
          setActiveView('posts');
          setActiveSection('posts-section');
          // Scroll to the targeted post element smoothly once rendered
          setTimeout(() => {
  ...
  }, [loading, uiTexts.hidePostsSection]);

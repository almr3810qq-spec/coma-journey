...
import { Download, BookOpen, Calendar, HelpCircle, Star, ZoomIn } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div 
        onClick={() => {
          if (book.coverImage) {
            setFullscreenImage(book.coverImage);
          }
        }}
        className={`relative mx-auto md:mx-0 flex h-72 w-48 shrink-0 select-none overflow-hidden rounded-sm border border-white/10 shadow-2xl transition-all duration-500 group-hover:scale-[1.03] ${book.coverImage ? 'cursor-pointer' : ''}`} 
        style={coverStyle}
      >

...
import { 
  ...
  Sun, Moon, Type, Minus, RotateCcw, Mail, Sliders, Phone, MessageSquare, ThumbsUp, Zap, Calendar,
  Video, Film, Image as ImageIcon, Play, VolumeX
} from 'lucide-react';
...
interface AdminDashboardProps {
  ...
  onClose: () => void;
  uiTexts: UiTexts;
  onUpdateUiTexts?: (uiTexts: UiTexts) => void;
...
export default function AdminDashboard({
  ...
  const handleCompressCurrentVideo = async () => {
    if (!editedUiTexts.topCustomVideoUrl) return;
    setIsCompressingVideo(true);

    try {
      const videoSrc = editedUiTexts.topCustomVideoUrl;
      ...
      await new Promise<void>((resolve) => {
        video.onloadeddata = async () => {
          try {
            const targetWidth = Math.min(video.videoWidth || 1280, 854);
            const targetHeight = Math.round((targetWidth / (video.videoWidth || 16)) * (video.videoHeight || 9));
            ...
            const recorderOptions = typeof MediaRecorder !== 'undefined' && MediaRecorder.isTypeSupported('video/webm;codecs=vp8')
              ? { mimeType: 'video/webm;codecs=vp8', videoBitsPerSecond: 800000 }
              ...
              reader.onloadend = () => {
                if (reader.result) {
                  setEditedUiTexts(prev => ({
                    ...prev,
                    topCustomVideoUrl: reader.result as string,
                    topCustomVideoCompressed: true
                  }));
                }
            ...
            }, Math.min((video.duration || 4) * 1000, 6000));
          } catch (err) {
            console.warn('Video compression fallback:', err);
            setEditedUiTexts(prev => ({ ...prev, topCustomVideoCompressed: true }));
            setIsCompressingVideo(false);
            resolve();
          }
        };
  ...
  useEffect(() => {
    if (onUpdateUiTexts && editedUiTexts) {
      onUpdateUiTexts(editedUiTexts);
    }
  }, [editedUiTexts, onUpdateUiTexts]);
  ...
  const handleHeroVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 100 * 1024 * 1024) {
      alert(isRtl ? 'حجم ملف الفيديو كبير جداً. يجب أن يكون أقل من 100 ميجابايت.' : 'Video file is too large. Must be under 100MB.');
      return;
    }
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data.url) {
              setEditedUiTexts(prev => ({
                ...prev,
                topCustomVideoUrl: data.url,
                topCustomBgType: prev.topCustomBgType === 'animated_video' ? 'animated_video' : 'video'
              }));
              setIsVideoUploading(false);
              return;
            }
          ...
          console.warn('API media upload fallback:', err);
        }

        // Fallback to data URL if endpoint unavailable
        setEditedUiTexts(prev => ({
          ...prev,
          topCustomVideoUrl: fileData,
          topCustomBgType: prev.topCustomBgType === 'animated_video' ? 'animated_video' : 'video'
        }));
        setIsVideoUploading(false);
      };
  ...
  const handleSavePost = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('coma_admin_token');
    const createdAtTimestamp = postCreatedAt ? new Date(postCreatedAt).getTime() : Date.now();
  ...
  const handleEditPostStart = (post: Post) => {
    ...
    if (post.createdAt) {
      try {
        ...
        setPostCreatedAt(`${year}-${month}-${day}T${hours}:${minutes}`);
      } catch (e) {
        setPostCreatedAt('');
      }
    } else {
            ...
            <div className="space-y-6">
              ...
              {isEditingPost ? (
                <form onSubmit={handleSavePost} className="space-y-6 rounded-xl border border-slate-800 bg-brand-ink/40 p-6">
                  ...
                  {/* Publication Date & Time Control (Past or Future) */}
                  <div className="border-t border-slate-900 pt-5">
                    <label className="block text-xs text-brand-gold mb-2 font-mono font-bold uppercase tracking-wider flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-brand-gold" />
                      <span>{isRtl ? 'التحكم في تاريخ ووقت نشر المنشور (للماضي أو المستقبل)' : 'Publication Date & Time (Past or Future)'}</span>
                    </label>
                    <input
                      type="datetime-local"
                      value={postCreatedAt}
                      onChange={(e) => setPostCreatedAt(e.target.value)}
                      className="w-full sm:w-80 rounded-sm border border-slate-800 bg-brand-deep/50 px-3 py-2 text-xs text-white font-mono focus:border-brand-gold focus:outline-none"
                    />
                    <p className="mt-1.5 text-[11px] text-slate-400 font-mono">
                      {isRtl 
                        ? 'يمكنك تغيير تاريخ النشر إلى أي تاريخ سابق (أرشيفي) أو تاريخ مستقبلي للتحكم في ظهور المنشور وترتيبه.' 
                        : 'Set any past date (archival) or future date to control post publication timing and order.'}
                    </p>
                  </div>
            ...
            <form onSubmit={handleSaveUiTexts} className="space-y-8 animate-fade-in text-left">
              ...
              <div className="rounded-sm border border-slate-800 bg-brand-deep/20 p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* Hero Background Customization Panel (Image / Video / Never-Ending Animated Video) */}
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">
                      {isRtl ? 'خلفية قسم الترحيب الرئيسي (صورة / فيديو / فيديو متحرك متكرر)' : 'Hero Section Background (Image / Video / Looping Animated Video)'}
                    </label>
                    <p className="text-[10px] text-slate-500 mb-3 leading-normal">
                      {isRtl 
                        ? 'يمكنك تغيير نوع الخلفية بالكامل في الواجهة الرئيسية: اختيار صورة ثابتة، أو رفع فيديو خلفية، أو ضبط فيديو متحرك متكرر بلا نهاية (Loop Video).'
                        : 'Choose the background type for the main welcome section: static image, video background, or a never-ending looping animated video.'}
                    </p>
                    
                    <div className="space-y-4 rounded-sm border border-slate-800 bg-slate-900/60 p-4">
                      
                      {/* Interactive Live Hero Preview Card */}
                      <div className="relative overflow-hidden rounded-md border border-brand-gold/40 bg-brand-ink h-48 w-full flex flex-col items-center justify-center text-center p-4 shadow-2xl group">
                        <div className="absolute inset-0 z-0">
                          {((editedUiTexts.topCustomBgType === 'video' || editedUiTexts.topCustomBgType === 'animated_video') && (editedUiTexts.topCustomVideoUrl || 'https://assets.mixkit.co/videos/preview/mixkit-gold-particles-floating-in-the-dark-41527-large.mp4')) ? (
                            <video
                              key={`mini-prev-${editedUiTexts.topCustomVideoUrl}`}
                              src={editedUiTexts.topCustomVideoUrl || 'https://assets.mixkit.co/videos/preview/mixkit-gold-particles-floating-in-the-dark-41527-large.mp4'}
                              autoPlay
                              loop={editedUiTexts.topCustomVideoLoop !== false}
                              muted={editedUiTexts.topCustomVideoMuted !== false}
                              playsInline
                              className="w-full h-full object-cover pointer-events-none"
                              style={{
                                opacity: (editedUiTexts.topCustomImageOpacity !== undefined ? editedUiTexts.topCustomImageOpacity : 80) / 100,
                                filter: `blur(${editedUiTexts.topCustomImageBlur || 0}px) contrast(${editedUiTexts.topCustomImageContrast !== undefined ? editedUiTexts.topCustomImageContrast : 100}%) brightness(${editedUiTexts.topCustomImageBrightness !== undefined ? editedUiTexts.topCustomImageBrightness : 100}%)`
                              }}
                            />
                          ) : editedUiTexts.topCustomImageUrl ? (
                            <img
                              src={editedUiTexts.topCustomImageUrl}
                              alt="Preview"
                              className="w-full h-full object-cover pointer-events-none"
                              style={{
                                opacity: (editedUiTexts.topCustomImageOpacity !== undefined ? editedUiTexts.topCustomImageOpacity : 80) / 100,
                            ...
                            <div className="w-full h-full bg-brand-ink flex items-center justify-center text-slate-500 text-xs font-mono">
                              {isRtl ? 'خلفية الإضاءة الافتراضية' : 'Default Ambient Beam Background'}
                            </div>
                          )}
                        ...
                        <div className="relative z-10 space-y-1 max-w-sm">
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[9px] font-bold uppercase tracking-widest text-brand-gold bg-black/80 rounded border border-brand-gold/40 shadow-lg">
                            <Sparkles className="h-3 w-3 text-brand-gold" />
                            <span>
                              {editedUiTexts.topCustomBgType === 'animated_video' 
                                ? (isRtl ? 'معاينة حية: فيديو متحرك متكرر' : 'Live Preview: Animated Loop')
                                : editedUiTexts.topCustomBgType === 'video'
                                ? (isRtl ? 'معاينة حية: فيديو خلفية' : 'Live Preview: Video Background')
                                : (isRtl ? 'معاينة حية: صورة ثابتة' : 'Live Preview: Static Image')}
                            </span>
                          </div>
                          <h3 className="font-serif text-base md:text-lg font-bold text-white drop-shadow-md">
                            {isRtl ? (editedUiTexts.heroTitleAr || 'عنوان الواجهة الرئيسية') : (editedUiTexts.heroTitleEn || 'Main Hero Title')}
                          </h3>
                          <p className="text-[10px] text-brand-gold italic font-serif line-clamp-1">
                            {isRtl ? (editedUiTexts.heroSubtitleAr || 'العنوان الفرعي للموقع') : (editedUiTexts.heroSubtitleEn || 'Author Subtitle')}
                          </p>
                        </div>
                      </div>

                      {/* 1. Background Type Selection Tabs */}
                      <div className="grid grid-cols-3 gap-1.5 p-1 bg-black/60 rounded border border-slate-800">
                        <button
                          type="button"
                          onClick={() => setEditedUiTexts(prev => ({ ...prev, topCustomBgType: 'image' }))}
                          className={`flex items-center justify-center gap-1.5 py-2 px-2 text-[11px] font-bold rounded transition-all cursor-pointer ${
                            (editedUiTexts.topCustomBgType || 'image') === 'image'
                              ? 'bg-brand-gold text-black shadow-md'
                              : 'text-slate-400 hover:text-white hover:bg-white/5'
                          }`}
                        >
                          <ImageIcon className="h-3.5 w-3.5 shrink-0" />
                          <span>{isRtl ? 'صورة ثابتة' : 'Image'}</span>
                        </button>

                        <button
                          type="button"
                          onClick={() => setEditedUiTexts(prev => ({
                            ...prev,
                            topCustomBgType: 'video',
                            topCustomVideoUrl: prev.topCustomVideoUrl || 'https://assets.mixkit.co/videos/preview/mixkit-gold-particles-floating-in-the-dark-41527-large.mp4'
                          }))}
                          className={`flex items-center justify-center gap-1.5 py-2 px-2 text-[11px] font-bold rounded transition-all cursor-pointer ${
                            editedUiTexts.topCustomBgType === 'video'
                              ? 'bg-brand-gold text-black shadow-md'
                        ...
                        >
                          ...
                          <span>{isRtl ? 'فيديو خلفية' : 'Video'}</span>
                        </button>
                        ...
                        <button
                          ...
                          onClick={() => setEditedUiTexts(prev => ({
                            ...prev,
                            topCustomBgType: 'animated_video',
                            topCustomVideoLoop: true,
                            topCustomVideoUrl: prev.topCustomVideoUrl || 'https://assets.mixkit.co/videos/preview/mixkit-gold-particles-floating-in-the-dark-41527-large.mp4'
                          }))}
                          className={`flex items-center justify-center gap-1.5 py-2 px-2 text-[11px] font-bold rounded transition-all cursor-pointer ${
                            editedUiTexts.topCustomBgType === 'animated_video'
                        ...
                        >
                          ...
                          <span>{isRtl ? 'فيديو متحرك متكرر' : 'Animated Loop'}</span>
                        </button>
                      </div>

                      {/* 2. IMAGE MODE CONTROLS */}
                      {((editedUiTexts.topCustomBgType || 'image') === 'image') && (
                        <div className="space-y-3 pt-1">
                          <div className="flex items-center gap-4">
                            <div className="relative flex-1">
                              ...
                              >
                                <Upload className="h-4 w-4 text-brand-gold" />
                                <span>{isRtl ? 'رفع ملف صورة الخلفية' : 'Upload Image File'}</span>
                              </label>
                            </div>
                            ...
                            {editedUiTexts.topCustomImageUrl ? (
                              ...
                              <span className="text-[10px] text-slate-500 font-mono italic">{isRtl ? 'لا توجد صورة' : 'No Image'}</span>
                            )}
                          </div>

                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-slate-500 uppercase shrink-0">رابط / URL:</span>
                            <input
                              type="text"
                      ...
                      {/* 3. VIDEO / ANIMATED VIDEO MODE CONTROLS */}
                      {(editedUiTexts.topCustomBgType === 'video' || editedUiTexts.topCustomBgType === 'animated_video') && (
                        <div className="space-y-3 pt-1">
                          
                          {/* Upload Video or Direct URL */}
                          {isVideoUploading ? (
                            <div className="flex items-center justify-center gap-2 p-3 rounded bg-brand-gold/10 border border-brand-gold/40 text-xs text-brand-gold font-bold animate-pulse">
                              <RefreshCw className="h-4 w-4 animate-spin text-brand-gold" />
                              <span>{isRtl ? 'جاري رفع ملف الفيديو وتخزينه بالسيرفر...' : 'Uploading video file to server...'}</span>
                            </div>
                          ) : (
                            <div className="flex items-center gap-4">
                              <div className="relative flex-1">
                                <input
                                  ...
                                  className="hidden"
                                  id="hero-bg-video-input"
                                />
                                <label
                                  htmlFor="hero-bg-video-input"
                                  className="flex items-center justify-center gap-2 rounded bg-slate-800 hover:bg-slate-700 px-4 py-2.5 text-xs font-bold text-slate-200 cursor-pointer transition-colors w-full"
                                >
                                  <Upload className="h-4 w-4 text-brand-gold" />
                                  <span>{isRtl ? 'رفع ملف فيديو (MP4/WebM)' : 'Upload Video File'}</span>
                                </label>
                              </div>

                              {editedUiTexts.topCustomVideoUrl ? (
                                <div className="flex items-center gap-2">
                                  <div className="w-10 h-10 border border-brand-gold/30 rounded bg-black/40 flex items-center justify-center overflow-hidden shrink-0">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setEditedUiTexts({ ...editedUiTexts, topCustomVideoUrl: '' })}
                                    className="p-1.5 bg-brand-crimson/10 hover:bg-brand-crimson text-brand-crimson hover:text-white rounded transition-all cursor-pointer"
                                ...
                                <span className="text-[10px] text-slate-500 font-mono italic">{isRtl ? 'لا يوجد فيديو' : 'No Video'}</span>
                              )}
                            </div>
                          )}
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-slate-500 uppercase shrink-0">رابط الفيديو:</span>
                            <input
                              type="text"
                              placeholder={isRtl ? 'أدخل رابط الفيديو المباشر (.mp4 / .webm)...' : 'Paste direct video link (.mp4 / .webm)...'}
                              value={editedUiTexts.topCustomVideoUrl || ''}
                              onChange={e => setEditedUiTexts({ ...editedUiTexts, topCustomVideoUrl: e.target.value })}
                              className="w-full rounded-sm border border-slate-800 bg-black/40 p-2 text-xs text-white focus:border-brand-gold focus:outline-none"
                            />
                          </div>

                          {/* Quick Presets for Never-Ending Animated Video Loops */}
                          <div className="rounded border border-slate-800 bg-black/40 p-2.5 space-y-2">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-bold text-brand-gold uppercase tracking-wider flex items-center gap-1">
                                <Sparkles className="h-3 w-3" />
                                {isRtl ? 'مقاطع فيديو متحركة متكررة بلا نهاية جاهزة (Preset Animated Loops)' : 'Ready Animated Loop Presets'}
                              </span>
                              <span className="text-[9px] text-slate-500">{isRtl ? 'اختر للتعيين المباشر' : 'Click to apply instantly'}</span>
                            </div>

                            <div className="grid grid-cols-2 gap-1.5">
                              <button
                                type="button"
                                onClick={() => setEditedUiTexts({
                                  ...editedUiTexts,
                                  topCustomVideoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-gold-particles-floating-in-the-dark-41527-large.mp4',
                                  topCustomBgType: 'animated_video',
                                  topCustomVideoLoop: true
                                })}
                                className="flex items-center gap-1.5 p-1.5 rounded bg-slate-800/80 hover:bg-brand-gold/20 hover:border-brand-gold/50 border border-slate-700/60 text-[10px] text-slate-200 text-right cursor-pointer transition-all"
                              >
                                <Play className="h-3 w-3 text-brand-gold shrink-0" />
                                <span className="truncate">{isRtl ? '🌌 غبار الكواكب والجسيمات الذهبية' : '🌌 Gold Particle Nebula'}</span>
                              </button>
                              ...
                              <button
                                ...
                                onClick={() => setEditedUiTexts({
                                  ...editedUiTexts,
                                  topCustomVideoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-abstract-gold-and-black-background-41530-large.mp4',
                                  topCustomBgType: 'animated_video',
                                  topCustomVideoLoop: true
                                })}
                                className="flex items-center gap-1.5 p-1.5 rounded bg-slate-800/80 hover:bg-brand-gold/20 hover:border-brand-gold/50 border border-slate-700/60 text-[10px] text-slate-200 text-right cursor-pointer transition-all"
                              >
                                <Play className="h-3 w-3 text-brand-gold shrink-0" />
                                <span className="truncate">{isRtl ? '✨ بريق وضياء سينمائي متحرك' : '✨ Cinematic Gold Wave'}</span>
                              </button>
                              ...
                              <button
                                ...
                                onClick={() => setEditedUiTexts({
                                  ...editedUiTexts,
                                  topCustomVideoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-stars-in-space-background-1610-large.mp4',
                                  topCustomBgType: 'animated_video',
                                  topCustomVideoLoop: true
                                })}
                                className="flex items-center gap-1.5 p-1.5 rounded bg-slate-800/80 hover:bg-brand-gold/20 hover:border-brand-gold/50 border border-slate-700/60 text-[10px] text-slate-200 text-right cursor-pointer transition-all"
                              >
                                <Play className="h-3 w-3 text-brand-gold shrink-0" />
                                <span className="truncate">{isRtl ? '🌊 سديم مظلم ونجوم هادئة' : '🌊 Dark Cosmic Stars'}</span>
                              </button>
                              ...
                              <button
                                ...
                                onClick={() => setEditedUiTexts({
                                  ...editedUiTexts,
                                  topCustomVideoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-white-dust-particles-floating-in-the-air-41529-large.mp4',
                                  topCustomBgType: 'animated_video',
                                  topCustomVideoLoop: true
                                })}
                                className="flex items-center gap-1.5 p-1.5 rounded bg-slate-800/80 hover:bg-brand-gold/20 hover:border-brand-gold/50 border border-slate-700/60 text-[10px] text-slate-200 text-right cursor-pointer transition-all"
                              >
                                <Play className="h-3 w-3 text-brand-gold shrink-0" />
                                <span className="truncate">{isRtl ? '📜 ذرات ضباب وحبر أدبي سينمائي' : '📜 Literary Dust Motion'}</span>
                              </button>
                            </div>
                          </div>

                          {/* Video Toggles & Settings */}
                          <div className="grid grid-cols-2 gap-3 pt-1">
                            {/* Loop Toggle */}
                            <label className="flex items-center gap-2 p-2 rounded bg-slate-800/50 border border-slate-800 cursor-pointer text-xs text-slate-300">
                              <input
                                type="checkbox"
                                checked={editedUiTexts.topCustomVideoLoop !== false}
                                onChange={e => setEditedUiTexts({ ...editedUiTexts, topCustomVideoLoop: e.target.checked })}
                                className="accent-brand-gold rounded"
                              />
                              <span>{isRtl ? '♾️ تكرار مستمر بلا نهاية' : 'Endless Video Loop'}</span>
                            </label>
                            ...
                            <label className="flex items-center gap-2 p-2 rounded bg-slate-800/50 border border-slate-800 cursor-pointer text-xs text-slate-300">
                              <input
                                type="checkbox"
                                checked={editedUiTexts.topCustomVideoMuted !== false}
                                onChange={e => setEditedUiTexts({ ...editedUiTexts, topCustomVideoMuted: e.target.checked })}
                                className="accent-brand-gold rounded"
                              />
                              <span>{isRtl ? '🔇 كتم الصوت لتشغيل تلقائي' : 'Mute Video (Auto Play)'}</span>
                            </label>
                          </div>

                          {/* Video Compression & Performance Optimization Section */}
                          <div className="p-3 rounded border border-brand-gold/30 bg-brand-gold/5 space-y-2.5">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <Zap className="h-4 w-4 text-brand-gold animate-pulse" />
                                <span className="text-xs font-bold text-brand-gold">
                                  {isRtl ? 'خيارات ضغط الفيديو وتحسين الأداء (Video Compression & Performance)' : 'Video Compression & Lightweight Mode'}
                                </span>
                              </div>
                              <span className="text-[9px] px-1.5 py-0.5 rounded bg-brand-gold/20 text-brand-gold font-mono font-bold">
                                {isRtl ? 'تسريع تحميل الموقع' : 'Ultra Fast'}
                              </span>
                            </div>

                            <p className="text-[11px] text-slate-300 leading-relaxed">
                              {isRtl
                                ? 'يسمح ضغط الفيديو وتقليل البث بتقديم خلفية خفيفة وسلسة جداً دون إبطاء تحمُّل صفحات الموقع أو استهلاك بيانات الزائر.'
                                : 'Compressing video keeps background playback light and silky smooth without degrading visual quality or slowing down page load.'}
                            </p>

                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1">
                              <label className="flex-1 flex items-center gap-2 p-2 rounded bg-slate-900 border border-slate-700/80 cursor-pointer text-xs text-slate-200">
                                <input
                                  type="checkbox"
                                  checked={editedUiTexts.topCustomVideoCompressed !== false}
                                  onChange={e => setEditedUiTexts({ ...editedUiTexts, topCustomVideoCompressed: e.target.checked })}
                                  className="accent-brand-gold rounded"
                                />
                                <span className="font-semibold">{isRtl ? '⚡ تفعيل نمط الفيديو الخفيف والمكتنز' : 'Enable Compressed Lightweight Mode'}</span>
                              </label>

                              <button
                                type="button"
                                disabled={isCompressingVideo || !editedUiTexts.topCustomVideoUrl}
                                onClick={handleCompressCurrentVideo}
                                className={`flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded transition-all cursor-pointer border ${
                                  isCompressingVideo
                                    ? 'bg-brand-gold/20 text-brand-gold border-brand-gold/40 animate-pulse cursor-wait'
                                    : !editedUiTexts.topCustomVideoUrl
                                    ? 'bg-slate-800 text-slate-600 border-slate-800 cursor-not-allowed'
                                    : 'bg-brand-gold text-black hover:bg-brand-gold/90 border-brand-gold shadow-md'
                                }`}
                              >
                                <Zap className={`h-3.5 w-3.5 ${isCompressingVideo ? 'animate-spin' : ''}`} />
                                <span>
                                  {isCompressingVideo
                                    ? (isRtl ? 'جاري ضغط الفيديو...' : 'Compressing Video...')
                                    : (isRtl ? 'ضغط واختزال الفيديو الحالي' : 'Compress Current Video')}
                                </span>
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="space-y-1 pt-1">
                            <div className="flex items-center justify-between text-[10px]">
                              <span className="text-slate-400 font-medium">{isRtl ? 'سرعة الحركة والتكرار' : 'Video Playback Speed'}</span>
                              <span className="font-mono text-brand-gold font-bold">{editedUiTexts.topCustomVideoSpeed || 1}x</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              {[0.5, 0.75, 1.0, 1.25, 1.5].map(spd => (
                                <button
                                  key={spd}
                                  type="button"
                                  onClick={() => setEditedUiTexts({ ...editedUiTexts, topCustomVideoSpeed: spd })}
                                  className={`flex-1 py-1 text-[10px] font-mono font-bold rounded border transition-all cursor-pointer ${
                                    (editedUiTexts.topCustomVideoSpeed || 1) === spd
                          ...
                          {/* Poster Image / Fallback image option */}
                          <div className="pt-1">
                            <span className="block text-[10px] font-bold text-slate-400 mb-1">
                              {isRtl ? 'صورة غلاف مؤقتة (Poster Image fallback)' : 'Video Poster Fallback Image'}
                            </span>
                            <div className="flex items-center gap-2">
                              <input
                                type="text"
                                placeholder={isRtl ? 'رابط صورة الغلاف الاحتياطية...' : 'Optional poster image URL...'}
                                value={editedUiTexts.topCustomImageUrl || ''}
                                onChange={e => setEditedUiTexts({ ...editedUiTexts, topCustomImageUrl: e.target.value })}
                                className="w-full rounded-sm border border-slate-800 bg-black/40 p-2 text-xs text-white focus:border-brand-gold focus:outline-none"
                              />
                      ...
                      {/* 4. SHARED FILTERS & ADJUSTMENT SLIDERS */}
                      {(editedUiTexts.topCustomImageUrl || editedUiTexts.topCustomVideoUrl) && (
                        <div className="mt-3 p-3 rounded-sm border border-slate-800/80 bg-black/50 space-y-3.5">
                          <div className="flex items-center justify-between border-b border-slate-800 pb-1.5 mb-1">
                            <span className="text-[10px] font-bold text-brand-gold uppercase tracking-wider">
                              {isRtl ? 'إعدادات ضبط تباين ومظهر الخلفية' : 'Professional Contrast & Background Filters'}
                            </span>
                          ...
                          {/* Grid Pattern Overlay Toggle */}
                          <div className="pt-2 border-t border-slate-800/80">
                            <label className="flex items-center gap-2 p-2 rounded bg-slate-900 border border-slate-800 cursor-pointer text-xs text-slate-300">
                              <input
                                type="checkbox"
                                checked={Boolean(editedUiTexts.topCustomShowGridPattern)}
                                onChange={e => setEditedUiTexts({ ...editedUiTexts, topCustomShowGridPattern: e.target.checked })}
                                className="accent-brand-gold rounded"
                              />
                              <span>{isRtl ? '🏁 إظهار شبكة المربعات خلف النص (Background Grid Squares Pattern)' : 'Show Background Grid Squares'}</span>
                            </label>
                          </div>
                          <div className="space-y-1">
                            <div className="flex items-center justify-between text-[10px]">
                              <span className="text-slate-400 font-medium">{isRtl ? 'شفافية الخلفية (Opacity)' : 'Background Opacity'}</span>
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center justify-between text-[10px]">
                              <span className="text-slate-400 font-medium">{isRtl ? 'درجة السطوع (Brightness)' : 'Brightness'}</span>
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center justify-between text-[10px]">
                              <span className="text-slate-400 font-medium">{isRtl ? 'مستوى التباين (Contrast)' : 'Contrast'}</span>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase text-slate-400 mb-2 flex justify-between items-center">
                      <span className="text-brand-gold font-bold">{isRtl ? 'الشارة أو العبارة العلوية فوق العنوان (اختياري)' : 'Hero Accent Badge / Tagline (Optional)'}</span>
                      <span className="rounded bg-brand-gold/20 px-2 py-0.5 text-[10px] text-white border border-brand-gold/30 font-sans font-medium">
                        {isRtl ? 'الشارة العلوية' : 'Edit Badge'}
                      </span>
                    </label>

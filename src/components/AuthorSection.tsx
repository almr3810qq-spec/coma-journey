...
import { 
  ...
  Video, Phone, Globe, X, Linkedin, Youtube, Send, MessageSquare, AtSign, Disc, Smile, Twitch, Github, ZoomIn
} from 'lucide-react';
            ...
            <div className={`flex flex-col ${hasVideo ? 'md:flex-row lg:flex-col xl:flex-row' : ''} gap-6 justify-center items-center w-full`}>
              ...
              <div className="relative group overflow-hidden rounded-sm border border-white/10 bg-brand-deep p-4 shadow-2xl transition-all duration-500 hover:border-brand-gold shrink-0">
                <div className="relative h-96 w-72 overflow-hidden rounded-sm bg-[#0c0c0c] flex items-center justify-center">
                  ...
                  {images.length > 0 ? (
                    <div 
                      onClick={() => setFullscreenImage(images[currentIdx])}
                      className="relative h-full w-full cursor-pointer group/authorimg"
                    >
                      <AnimatePresence mode="wait">
                        <motion.img
                          ...
                          className="h-full w-full object-cover transition-transform duration-300 group-hover/authorimg:scale-105"
                      ...
                      <div className="absolute top-3 right-3 bg-black/80 text-brand-gold p-1.5 rounded-full opacity-0 group-hover/authorimg:opacity-100 transition-opacity border border-brand-gold/40 shadow-lg">
